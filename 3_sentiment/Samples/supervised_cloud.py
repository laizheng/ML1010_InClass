...
print("#build train and test datasets")
...
le = LabelEncoder()
num_classes=2
print("#tokenize train reviews & encode train labels")
tokenized_train = [tn.tokenizer.tokenize(text)
                   for text in norm_train_reviews]
y_tr = le.fit_transform(norm_train_sentiments)
y_train = keras.utils.to_categorical(y_tr, num_classes)
print("#tokenize test reviews & encode test labels")
tokenized_test = [tn.tokenizer.tokenize(text)
                   for text in norm_test_reviews]
y_ts = le.fit_transform(norm_train_sentiments)
y_test = keras.utils.to_categorical(y_ts, num_classes)

print("#Feature Engineering with word embeddings")
...
print("#generate averaged word vector features from word2vec model")
avg_wv_train_features = averaged_word2vec_vectorizer(corpus=tokenized_train, model=w2v_model,
                                                     num_features=500)
avg_wv_test_features = averaged_word2vec_vectorizer(corpus=tokenized_test, model=w2v_model,
                                                    num_features=500)

print("#feature engineering with GloVe model")
